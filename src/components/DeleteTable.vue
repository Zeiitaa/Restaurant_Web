...
const confirm = async () => {
    ...
    error.value = ''
    try {
        await api.delete(`/table/${props.item.table_code}`, {
            headers: { Authorization: `Bearer ${auth.token}` }
        })
        emit('deleted')
        close()
    } catch (e) {
        error.value = e.response?.data?.detail ?? 'Failed to delete table.'
    } finally {
            ...
            <div class="bg-white dark:bg-slate-800 w-full max-w-sm rounded-2xl shadow-2xl p-6 flex flex-col gap-5">
                ...
                <div class="flex flex-col items-center text-center gap-3">
                    <div class="size-14 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center">
                        <span class="material-symbols-outlined text-3xl text-red-500">table_restaurant</span>
                    </div>
                    <div>
                        <h3 class="text-lg font-black text-slate-900 dark:text-white">Delete Table?</h3>
                        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
                            Table
                            <span class="font-semibold text-slate-700 dark:text-slate-200">{{ item?.table_code }}</span>
